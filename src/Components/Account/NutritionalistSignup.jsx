import React from 'react'
import Logo from "../../asset/Logo.png"
import Subtract from "../../asset/Subtract.png"
import { Link } from "react-router-dom";

const NutritionalistSignup = () => {
  return (
    <div className="relative min-h-screen flex font-sans">
    <div className="flex flex-col sm:flex-row items-center md:items-start sm:justify-center md:justify-start flex-auto min-w-0 bg-white">
      <div className="sm:w-1/2 xl:w-1/2 h-full hidden md:flex flex-auto items-center justify-center p-10 overflow-hidden bg-purple-900 text-white bg-no-repeat bg-cover relative">
  
        <div className="absolute bg-[#EFFDE2] inset-0 z-0"></div>
        <div className="w-full  max-w-md z-10 shadow-none">
          <div className="sm:text-4xl xl:text-5xl font-bold leading-tight mb-6 -my-44">
            <img src={Logo} alt="" />
          </div>
          <div className="sm:text-sm xl:text-md text-gray-200 font-normal  "><img src={Subtract} className="object-cover  " alt=""/> </div>
        </div>
  
      </div>
      <div className="md:flex md:items-center md:justify-center w-full sm:w-auto md:h-full  xl:w-2/5 p-8  md:p-10 lg:p-14 sm:rounded-lg md:rounded-none bg-white">
        <div className="max-w-md w-full space-y-6">
          <div className="text-center">
            <h2 className="mt-6 text-3xl font-bold text-gray-900">
              Welcome !
            </h2>
            <p className="mt-2 text-13xl  text-black font-sans font-semibold">Create Nutritionalist Account</p>
          </div>
  
          <div class="flex flex-row justify-center items-center space-x-3 ">
              <a href="https://www.behance.net/" target="_blank"
                class="w-11 h-11 items-center justify-center inline-flex rounded-2xl text-blacl font-bold text-lg  bg-dark-green-color hover:shadow-lg cursor-pointer transition ease-in duration-300"><img
                  className="w-4 h-4"
                  src="data:image/svg+xml;base64,"/></a>
              <a href="https://twitter.com/" target="_blank"
                class="w-11 h-11 items-center justify-center inline-flex rounded-2xl font-bold text-lg  text-white bg-dark-green-color hover:shadow-lg cursor-pointer transition ease-in duration-300"><img
                  className="w-4 h-4"
                  src="data:image/svg+xml;base64,"/></a>
              <a href="https://in.linkedin.com/" target="_blank"
                class="w-11 h-11 items-center justify-center inline-flex rounded-2xl font-bold text-lg   text-white bg-dark-green-color hover:shadow-lg bg-blend-color-dodge cursor-pointer transition ease-in duration-300"><img
                  src="data:image/png;base64,"
                  className="w-4 h-4 mix-blend-contrast rounded-full"/></a>
            </div>
  
  
          <div className="flex items-center justify-center space-x-2">
            <span className="h-px w-16 bg-gray-200"></span>
            <span className="text-gray-300 font-normal">or continue with</span>
            <span className="h-px w-16 bg-gray-200"></span>
          </div>
          <form className="mt-8 space-y-6" action="" method="POST">
            <div>
            <label htmlFor="Name" className="ml-3 font-bold text-black text-5xl">Name</label>
            <input type="text" name="remember " className="w-full text-base px-4 py-2 border border-solid border-grey-color rounded-lg outline-none  focus:border-[#CFF7AA]"
                 placeholder="Enter Your Name" />
            </div>
            <div className="relative">
              <label htmlFor="email" className="ml-3  font-bold text-black text-5xl ">Email-ID</label>
              <input
                className="w-full text-base px-4 py-2 border border-solid border-grey-color rounded-lg outline-none  focus:border-[#CFF7AA]"
                type="email" placeholder="mail@gmail.com"  />
            </div>
            <div className="mt-8 content-center">
              <label htmlFor="password" className="ml-3 text-5xl font-bold text-gray-700 ">Phone No.</label>
              <input
                className="w-full content-center text-base px-4 py-2 border border-solid no no-number-spin border-grey-color rounded-lg  outline-none focus:border-[#CFF7AA]"
                type="Number" placeholder="1234567890" maxLength={10}/>
            </div>

            <div className="mt-8 content-center">
              <label htmlFor="password" className="ml-3 text-5xl font-bold text-gray-700 ">Password</label>
              <input
                className="w-full content-center text-base px-4 py-2 border border-solid border-grey-color rounded-lg  outline-none focus:border-[#CFF7AA]"
                type="password" placeholder="Enter your password" />
            </div>
        
            <div>
              <button
                type="submit"
                className="w-full flex justify-center mt-10 [background:linear-gradient(93.37deg,_rgba(175,_228,_126,_0.4),_rgba(216,_250,_185,_0.4))] border border-solid border-[#CFF7A] text-dark-green-color p-4 text-5xl  rounded-2xl tracking-wide font-semibold  cursor-pointer transition ease-in duration-500"
              >
                Sign Up
              </button>
            </div>
            <p className="flex  items-center justify-center mt-10 text-center text-md text-gray-500">
              <span>Already have an account?</span>
              <a href="/" className="text-dark-green-color hover:text-lime-600  font-semibold cursor-pointer transition ease-in duration-300">Log in</a>
            </p>
          </form>
        </div>
      </div>
    </div>
  </div>
  )
}

export default NutritionalistSignup